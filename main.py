...
# Viam Tipsy exercise code
# Robot logic for tipsy robot 
...
#synchronously loop through the readings to stop the base if it’s closer than a certain distance from an obstacle
...
# define a person detection loop where tipsy robot is constantly looking for a person - if it finds a person move it  toward them as long as there are no obstacles in front
...
# main function initializes the base, the sensors, and the detector. It also creates two background tasks running asynchronously, one looking for obstacles and avoiding them (obstacle_task), and one looking for people and moving towards them (person_task):
async def main():
    robot = await connect()
